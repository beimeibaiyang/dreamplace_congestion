...
class EvalMetrics (object):
    ...
    def __init__(self, iteration=None, detailed_step=None):
        ...
        self.wns = None
        self.eval_time = None
        self.avg_inflation_ratio = None
        self.max_inflation_ratio = None
        self.num_inflated_nodes = None
    ...
    def __str__(self):
        ...
        if self.avg_inflation_ratio is not None:
            content += ", AvgInflation %.6E" % (self.avg_inflation_ratio)
        if self.max_inflation_ratio is not None:
            content += ", MaxInflation %.6E" % (self.max_inflation_ratio)
        if self.num_inflated_nodes is not None:
            content += ", InflatedNodes %d" % (self.num_inflated_nodes)
    ...
    def evaluate(self, placedb, ops, var, data_collections=None):
        ...
        with torch.no_grad():
            ...
            if "inflation_stats" in ops:
                stats = ops["inflation_stats"]()
                self.avg_inflation_ratio = stats["avg_inflation_ratio"]
                self.max_inflation_ratio = stats["max_inflation_ratio"]
                self.num_inflated_nodes = stats["num_inflated_nodes"]

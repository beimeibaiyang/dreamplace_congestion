        ...
        if params.global_place_flag:
            ...
            for cur_stage, global_place_params in enumerate(global_place_stages):
                ...
                if params.routability_opt_flag:
                    ...
                    if params.enable_momentum_cell_inflation and self.op_collections.inflation_stats_op is not None:
                        eval_ops.update(
                            {
                                "inflation_stats": self.op_collections.inflation_stats_op,
                            }
                        )
                ...
                for Lgamma_step in range(model.Lgamma_iteration):
                    ...
                    for Llambda_density_weight_step in range(model.Llambda_density_weight_iteration):
                        ...
                        if (
                            ...
                            if (
                                params.enable_momentum_cell_inflation
                                and model.op_collections.momentum_inflation_op is not None
                                and route_utilization_map is not None
                            ):
                                inflation_applied = model.op_collections.momentum_inflation_op.update(
                                    pos=pos,
                                    route_utilization_map=route_utilization_map,
                                    global_round=num_area_adjust,
                                )
                                ...
                                if inflation_applied:
                                    ...
                                    logging.info(
                                        ...
                                        % (
                                            inflation_stats["avg_inflation_ratio"].item(),
                                            inflation_stats["max_inflation_ratio"].item(),
                                            int(inflation_stats["num_inflated_nodes"].item()),
                                        )
                                    )
            ...
            if params.routability_opt_flag:
                with torch.no_grad():
                    # if momentum inflation exists, restore its internal size modification first
                    # here anchor_centers=False because the original code below already converts
                    # lower-left -> center -> lower-left once.
                    if (
                        params.enable_momentum_cell_inflation
                        and self.op_collections.momentum_inflation_op is not None
                    ):
                        self.op_collections.momentum_inflation_op.restore_original_sizes(
                            anchor_centers=False
                        )
                    ...
                    # refresh cached data after restoration
                    self.data_collections.update_node_areas()
                    self.data_collections.update_movable_sorted_node_map(placedb)
